...
func main() {
	...
	if err := client.Start(); err != nil {
		log.Infof("failed to start driver: %v", err)
		os.Exit(1)
	}
	defer client.Close()
	...
	for _, f := range files {
		...
		if err != nil {
			log.Infof("cannot perform benchmark over the file %v: %v", f, err)
			os.Exit(1)
		}
	...
	if err != nil {
		log.Infof("failed to marshal results: %v", err)
		os.Exit(1)
	}
	...
	if _, err := f.Write(data); err != nil {
		log.Infof("failed to write to file %v: %v", file, err)
	}
}
